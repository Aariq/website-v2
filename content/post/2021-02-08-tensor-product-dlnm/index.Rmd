...
After re-reading Gasparrini's papers for the billionth time and reading more of Simon Wood's book, I realized the difference between the pure `mgcv` approach and the `dlnm` approach had to do with "identifiability constraints".  Basically, because a GAM is a function that has covariates which themselves are functions, those smooth covariates are usually constrained to sum to 0.
...
Ultimately, there is little difference between these approaches for these data.  If we use the `cb_margeff()` function I mentioned [earlier](post/dlnm-getting-started) on in this series to get fitted values of y (for the whole GAM, not just the smooth), then the two models look nearly identical.
